...
def create_example_agenda():
    ...
    agenda.add_event("cArr", "Surprise Challenge", "",
                      time_ranges=[(d_thu, "13:15", "13:30")]) # Adjusted to prevent overlap
...
# Create Agenda and Generate LaTeX
agenda = create_example_agenda()
if not use_legacy_render:
    latex_code = agenda.generate_latex_tikz(granularity_minutes=15, scale=0.48, width_pct=2.2, height_pct=1.4, gap_x_pct=0.05, gap_y_pct=0.0, ext_y_pct=0.3)
else:
