...
If one does not want to use the 'tikz' package one can use the `--legacy` option. To save the output directly to a file use `--file <filename>`. One can parse the agenda events from a csv file by using `--csv-file`.
